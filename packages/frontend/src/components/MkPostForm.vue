...
const visibleUsers = ref<Fedired.entities.UserDetailed[]>([]);
...
if (props.reply && ['home', 'followers', 'specified'].includes(props.reply.visibility)) {
	...
	if (visibility.value === 'specified') {
		...
		if (props.reply.userId !== $i.id) {
			fediredApi('users/show', { userId: props.reply.userId }).then(user => {
...
function addMissingMention() {
	...
	for (const x of extractMentions(ast)) {
		if (!visibleUsers.value.some(u => (u.username === x.username) && (u.host === x.host))) {
			fediredApi('users/show', { username: x.username, host: x.host }).then(user => {
				pushVisibleUser(user);
			});
		}
	}
...
function replaceFile(file: Fedired.entities.DriveFile, newFile: Fedired.entities.DriveFile): void {
...
function pushVisibleUser(user: Fedired.entities.UserDetailed) {
...
function addVisibleUser() {
	os.selectUser().then(user => {
		...
		if (!text.value.toLowerCase().includes(`@${user.username.toLowerCase()}`)) {
			text.value = `@${Fedired.acct.toString(user)} ${text.value}`;
		}
	});
}
...
async function post(ev?: MouseEvent) {
	...
	posting.value = true;
	fediredApi('notes/create', postData, token).then(() => {
		...
		nextTick(() => {
			...
			if ((lowerCase.includes('love') || lowerCase.includes('❤')) && lowerCase.includes('fedired')) {
...
function insertMention() {
	os.selectUser({ localOnly: localOnly.value, includeSelf: true }).then(user => {
		insertTextAtCursor(textareaEl.value, '@' + Fedired.acct.toString(user) + ' ');
	});
}
...
async function insertEmoji(ev: MouseEvent) {
	...
	// See: https://github.com/fedired-dev/fedired/pull/14282
	//      https://github.com/fedired-dev/fedired/issues/14274
...
const postAccount = ref<Fedired.entities.UserDetailed | null>(null);
	...
	nextTick(() => {
		...
		if (!props.instant && !props.mention && !props.specified && !props.mock) {
			...
			if (draft) {
				...
				if (draft.data.visibleUserIds) {
					fediredApi('users/show', { userIds: draft.data.visibleUserIds }).then(users => {
						users.forEach(u => pushVisibleUser(u));
					});
				}
		...
		if (props.initialNote) {
			...
			if (init.visibleUserIds) {
				fediredApi('users/show', { userIds: init.visibleUserIds }).then(users => {
					users.forEach(u => pushVisibleUser(u));
				});
			}
