...
const relays = ref<Fedired.entities.AdminRelaysListResponse>([]);
...
function remove(inbox: string) {
	fediredApi('admin/relays/remove', {
		inbox,
	}).then(() => {
		refresh();
	}).catch((err: any) => {
...
function refresh() {
	fediredApi('admin/relays/list').then(relayList => {
