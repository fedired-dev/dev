...
/**
 * Basic OAuth tests to make sure the library is correctly integrated to Fedired
...
function getMeta(html: string): { transactionId: string | undefined, clientName: string | undefined } {
	...
	return {
		transactionId: fragment.querySelector<HTMLMetaElement>('meta[name="fedired:oauth:transaction-id"]')?.content,
		clientName: fragment.querySelector<HTMLMetaElement>('meta[name="fedired:oauth:client-name"]')?.content,
	};
}

function fetchDecision(transactionId: string, user: fedired.entities.SignupResponse, { cancel }: { cancel?: boolean } = {}): Promise<Response> {
...
async function fetchDecisionFromResponse(response: Response, user: fedired.entities.SignupResponse, { cancel }: { cancel?: boolean } = {}): Promise<Response> {
...
async function fetchAuthorizationCode(user: fedired.entities.SignupResponse, scope: string, code_challenge: string): Promise<{ client: AuthorizationCode, code: string }> {
...
function assertIndirectError(response: Response, error: string): void {
	...
	assert.strictEqual(location.searchParams.get('iss'), 'http://fedired.local');
...
describe('OAuth', () => {
	...
	beforeEach(async () => {
		await sendEnvUpdateRequest({ key: 'FEDIREDTEST_CHECK_IP_RANGE', value: '' });
		sender = (reply): void => {
			reply.send(`
				<!DOCTYPE html>
	...
	test('Full flow', async () => {
		...
		const createResultBody = createResult.body as fedired.Endpoints['notes/create']['res'];
	...
	test('Two concurrent flows', async () => {
		...
		const createResultBodyAlice = await createResultAlice.body as fedired.Endpoints['notes/create']['res'];
		assert.strictEqual(createResultBodyAlice.createdNote.user.username, 'alice');

		const createResultBodyBob = await createResultBob.body as fedired.Endpoints['notes/create']['res'];
	...
	describe('Scope', () => {
		...
		// (Although Fedired always return scope, which is also fine)
		...
		test('Scope check by API', async () => {
			...
			assert.ok(createResult.headers.get('WWW-Authenticate')?.startsWith('Bearer realm="Fedired", error="insufficient_scope", error_description'));
		});
	});
	...
	test('Server metadata', async () => {
		...
		assert.strictEqual(body.issuer, 'http://fedired.local');
		assert.ok(body.scopes_supported.includes('write:notes'));
	});

	// Any error on decision endpoint is solely on Fedired side and nothing to do with the client.
	...
	describe('Client Information Discovery', () => {
		...
		test('Disallow loopback', async () => {
			await sendEnvUpdateRequest({ key: 'FEDIREDTEST_CHECK_IP_RANGE', value: '1' });
