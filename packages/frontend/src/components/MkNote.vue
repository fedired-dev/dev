...
const currentClip = inject<Ref<Fedired.entities.Clip> | null>('currentClip', null);
...
if (noteViewInterruptors.length > 0) {
	onMounted(async () => {
		let result: Fedired.entities.Note | null = deepClone(note.value);
		for (const interruptor of noteViewInterruptors) {
			try {
				result = await interruptor.handler(result!) as Fedired.entities.Note | null;
				if (result === null) {
		...
		note.value = result as Fedired.entities.Note;
	});
}

const isRenote = Fedired.note.isPureRenote(note.value);
...
const translation = ref<Fedired.entities.NotesTranslateResponse | null>(null);
const translating = ref(false);
const showTicker = (defaultStore.state.instanceTicker === 'always') || (defaultStore.state.instanceTicker === 'remote' && appearNote.value.user.instance);
const canRenote = computed(() => ['public', 'home'].includes(appearNote.value.visibility) || (appearNote.value.visibility === 'followers' && appearNote.value.userId === $i?.id));
const renoteCollapsed = ref(
	defaultStore.state.collapseRenotes && isRenote && (
		($i && ($i.id === note.value.userId || $i.id === appearNote.value.userId)) || // `||` must be `||`! See https://github.com/fedired-dev/fedired/issues/13131
...
function checkMute(noteToCheck: Fedired.entities.Note, mutedWords: Array<string | string[]> | undefined | null, checkOnly: true): boolean;
function checkMute(noteToCheck: Fedired.entities.Note, mutedWords: Array<string | string[]> | undefined | null, checkOnly: false): boolean | 'sensitiveMute';
*/
function checkMute(noteToCheck: Fedired.entities.Note, mutedWords: Array<string | string[]> | undefined | null, checkOnly = false): boolean | 'sensitiveMute' {
	if (mutedWords != null) {
...
provide('react', (reaction: string) => {
	fediredApi('notes/reactions/create', {
...
if (!props.mock) {
	useTooltip(renoteButton, async (showing) => {
		const renotes = await fediredApi('notes/renotes', {
			noteId: appearNote.value.id,
			limit: 11,
		});
	...
	if (appearNote.value.reactionAcceptance === 'likeOnly') {
		useTooltip(reactButton, async (showing) => {
			const reactions = await fediredApiGet('notes/reactions', {
				noteId: appearNote.value.id,
				limit: 10,
...
function undoReact(targetNote: Fedired.entities.Note): void {
	...
	fediredApi('notes/reactions/delete', {
