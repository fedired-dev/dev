...
describe('クリップ', () => {
	...
	let aliceHomeNote: Fedired.entities.Note;
	let aliceFollowersNote: Fedired.entities.Note;
	let aliceSpecifiedNote: Fedired.entities.Note;
	let bobNote: Fedired.entities.Note;
	let bobHomeNote: Fedired.entities.Note;
	let bobFollowersNote: Fedired.entities.Note;
	let bobSpecifiedNote: Fedired.entities.Note;

	const compareBy = <T extends { id: string }, >(selector: (s: T) => string = (s: T): string => s.id) => (a: T, b: T): number => {
		return selector(a).localeCompare(selector(b));
	};

	const defaultCreate = (): Pick<Fedired.entities.ClipsCreateRequest, 'name'> => ({
		name: 'test',
	});
	const create = async (parameters: Partial<Fedired.entities.ClipsCreateRequest> = {}, request: Partial<ApiRequest<'clips/create'>> = {}): Promise<Fedired.entities.Clip> => {
	...
	const createMany = async (parameters: Partial<Fedired.entities.ClipsCreateRequest>, count = 10, user = alice): Promise<Fedired.entities.Clip[]> => {
	...
	const update = async (parameters: Optional<Fedired.entities.ClipsUpdateRequest, 'name'>, request: Partial<ApiRequest<'clips/update'>> = {}): Promise<Fedired.entities.Clip> => {
		const clip = await successfulApiCall({
	...
	const deleteClip = async (parameters: Fedired.entities.ClipsDeleteRequest, request: Partial<ApiRequest<'clips/delete'>> = {}): Promise<void> => {
	...
	const show = async (parameters: Fedired.entities.ClipsShowRequest, request: Partial<ApiRequest<'clips/show'>> = {}): Promise<Fedired.entities.Clip> => {
	...
	const list = async (request: Partial<ApiRequest<'clips/list'>>): Promise<Fedired.entities.Clip[]> => {
	...
	const usersClips = async (parameters: Fedired.entities.UsersClipsRequest, request: Partial<ApiRequest<'users/clips'>> = {}): Promise<Fedired.entities.Clip[]> => {
		return await successfulApiCall({
	...
	test.each([
		...
		// 返ってくる配列には順序保障がないのでidでソートして厳密比較
		assert.deepStrictEqual(
			res.sort(compareBy<Fedired.entities.Clip>(s => s.id)),
			clips.sort(compareBy(s => s.id)));
	...
	describe('のお気に入り', () => {
		let aliceClip: Fedired.entities.Clip;

		const favorite = async (parameters: Fedired.entities.ClipsFavoriteRequest, request: Partial<ApiRequest<'clips/favorite'>> = {}): Promise<void> => {
			await successfulApiCall({
		...
		const unfavorite = async (parameters: Fedired.entities.ClipsUnfavoriteRequest, request: Partial<ApiRequest<'clips/unfavorite'>> = {}): Promise<void> => {
			await successfulApiCall({
		...
		const myFavorites = async (request: Partial<ApiRequest<'clips/my-favorites'>> = {}): Promise<Fedired.entities.Clip[]> => {
			return successfulApiCall({
	...
	describe('に紐づくノート', () => {
		...
		const sampleNotes = (): Fedired.entities.Note[] => [
			aliceNote, aliceHomeNote, aliceFollowersNote, aliceSpecifiedNote,
			bobNote, bobHomeNote, bobFollowersNote, bobSpecifiedNote,
		];

		const addNote = async (parameters: Fedired.entities.ClipsAddNoteRequest, request: Partial<ApiRequest<'clips/add-note'>> = {}): Promise<void> => {
			return successfulApiCall({
		...
		const removeNote = async (parameters: Fedired.entities.ClipsRemoveNoteRequest, request: Partial<ApiRequest<'clips/remove-note'>> = {}): Promise<void> => {
			return successfulApiCall({
		...
		const notes = async (parameters: Fedired.entities.ClipsNotesRequest, request: Partial<ApiRequest<'clips/notes'>> = {}): Promise<Fedired.entities.Note[]> => {
			return successfulApiCall({
		...
		test('をポリシーで定められた上限いっぱい(200)を超えて追加はできない。', async () => {
			...
			const noteList = await Promise.all([...Array(noteLimit)].map((_, i) => post(alice, {
				text: `test ${i}`,
			}) as unknown)) as Fedired.entities.Note[];
