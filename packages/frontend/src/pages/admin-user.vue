...
const user = ref<null | Fedired.entities.UserDetailed>();
const init = ref<ReturnType<typeof createFetcher>>();
const info = ref<any>();
const ips = ref<Fedired.entities.AdminGetUserIpsResponse | null>(null);
...
function createFetcher() {
	return () => Promise.all([fediredApi('users/show', {
		userId: props.userId,
	}), fediredApi('admin/show-user', {
		userId: props.userId,
	}), iAmAdmin ? fediredApi('admin/get-user-ips', {
		userId: props.userId,
		...
		watch(moderationNote, async () => {
			await fediredApi('admin/update-user-note', { userId: user.value.id, text: moderationNote.value });
			await refreshUser();
		});
	});
}
...
async function resetPassword() {
	...
	if (confirm.canceled) {
		...
		const { password } = await fediredApi('admin/reset-password', {
...
async function toggleSuspend(v) {
	...
	if (confirm.canceled) {
		...
		await fediredApi(v ? 'admin/suspend-user' : 'admin/unsuspend-user', { userId: user.value.id });
		await refreshUser();
	}
}
...
async function unsetUserAvatar() {
	...
	const process = async () => {
		await fediredApi('admin/unset-user-avatar', { userId: user.value.id });
		os.success();
	};
...
async function unsetUserBanner() {
	...
	if (confirm.canceled) return;
	const process = async () => {
		await fediredApi('admin/unset-user-banner', { userId: user.value.id });
		os.success();
	};
	await process().catch(err => {
		os.alert({
...
async function deleteAllFiles() {
	...
	if (confirm.canceled) return;
	const process = async () => {
		await fediredApi('admin/delete-all-files-of-a-user', { userId: user.value.id });
		os.success();
	};
	await process().catch(err => {
		os.alert({
...
async function assignRole() {
	const roles = await fediredApi('admin/roles/list');
