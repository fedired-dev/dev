...
describe('User', () => {
	describe('Profile', () => {
		...
		describe('Pinning Notes', () => {
			let alice: LoginUser, bob: LoginUser;
			let aliceInB: Fedired.entities.UserDetailedNotMe;
			...
			let pinnedNote: Fedired.entities.Note;
	...
	describe('Deletion', () => {
		...
		describe('Deletion of remote user for moderation', () => {
			...
			test('Bob follows Alice, then Alice gets deleted in B server', async () => {
				 ...
				 *        @see https://github.com/fedired-dev/fedired/issues/14728
				 */
	...
	describe('Suspension', () => {
		describe('Check suspend/unsuspend consistency', () => {
			...
			test('Alice gets unsuspended, Bob succeeds in following Alice', async () => {
				 ...
				 *        related @see https://github.com/fedired-dev/fedired/issues/13273
				 */
				await rejects(
					async () => await bob.client.request('following/create', { userId: aliceInB.id }),
					(err: any) => {
