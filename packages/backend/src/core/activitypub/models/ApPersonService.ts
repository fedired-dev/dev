...
export class ApPersonService implements OnModuleInit {
	 ...
	 *
	 * Fediredに対象のPersonが登録されていればそれを返し、登録がなければnullを返します。
	 */
	@bindThis
	...
	public async createPerson(uri: string, resolver?: Resolver): Promise<MiRemoteUser> {
		...
		try {
			...
			await this.db.transaction(async transactionalEntityManager => {
				...
				if (person._fedired_summary) {
					_description = truncate(person._fedired_summary, summaryLength);
				...
				await transactionalEntityManager.save(new MiUserProfile({
					...
					followedMessage: person._fedired_followedMessage != null ? truncate(person._fedired_followedMessage, 256) : null,
					url,
					fields,
					followingVisibility,
					followersVisibility,
	...
	/**
	 * Personの情報を更新します。
	 * Fediredに対象のPersonが登録されていなければ無視します。
	 ...
	 * Personを解決します。
	 *
	 * Fediredに対象のPersonが登録されていればそれを返し、そうでなければ
