...
describe('Note', () => {
	...
	let tom: fedired.entities.SignupResponse;
	...
	test('ファイルを添付できる', async () => {
		const file = await uploadUrl(alice, 'https://raw.githubusercontent.com/fedired-dev/fedired/develop/packages/backend/test/resources/192.jpg');
	...
	test('他人のファイルで怒られる', async () => {
		const file = await uploadUrl(bob, 'https://raw.githubusercontent.com/fedired-dev/fedired/develop/packages/backend/test/resources/192.jpg');
	...
	describe('notes/translate', () => {
		describe('翻訳機能の利用が許可されていない場合', () => {
			let cannotTranslateRole: fedired.entities.Role;
