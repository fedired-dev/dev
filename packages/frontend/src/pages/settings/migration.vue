...
const movedTo = ref<Fedired.entities.UserDetailed>();
const accountAliases = ref(['']);

async function init() {
	if ($i.movedTo) {
		movedTo.value = await fediredApi('users/show', { userId: $i.movedTo });
	} else {
		moveToAccount.value = '';
	}

	if ($i.alsoKnownAs && $i.alsoKnownAs.length > 0) {
		const alsoKnownAs = await fediredApi('users/show', { userIds: $i.alsoKnownAs });
		accountAliases.value = (alsoKnownAs && alsoKnownAs.length > 0) ? alsoKnownAs.map(user => `@${Fedired.acct.toString(user)}`) : [''];
	} else {
