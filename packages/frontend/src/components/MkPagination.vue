...
import { FediredEntity } from '@/types/date-separated-list.js';
...
export type Paging<E extends keyof Fedired.Endpoints = keyof Fedired.Endpoints> = {
	endpoint: E;
	limit: number;
	params?: Fedired.Endpoints[E]['req'] | ComputedRef<Fedired.Endpoints[E]['req']>;
...
type FediredEntityMap = Map<string, FediredEntity>;

function arrayToEntries(entities: FediredEntity[]): [string, FediredEntity][] {
	return entities.map(en => [en.id, en]);
}

function concatMapWithArray(map: FediredEntityMap, entities: FediredEntity[]): FediredEntityMap {
 ...
 * 最新が0番目
 */
const items = ref<FediredEntityMap>(new Map());

/**
 * タブが非アクティブなどの場合に更新を貯めておく
 * 最新が0番目
 */
const queue = ref<FediredEntityMap>(new Map());
...
async function init(): Promise<void> {
	...
	await fediredApi<FediredEntity[]>(props.pagination.endpoint, {
		...params,
 ...
 * @param item アイテム
 */
const prepend = (item: FediredEntity): void => {
 ...
 */
function unshiftItems(newItems: FediredEntity[]) {
 ...
 */
function concatItems(oldItems: FediredEntity[]) {
...
function prependQueue(newItem: FediredEntity) {
	queue.value = new Map([[newItem.id, newItem], ...queue.value].slice(0, props.displayLimit) as [string, FediredEntity][]);
}

/*
 * アイテムを末尾に追加する（使うの？）
 */
const appendItem = (item: FediredEntity): void => {
	items.value.set(item.id, item);
};
...
const updateItem = (id: FediredEntity['id'], replacer: (old: FediredEntity) => FediredEntity): void => {
