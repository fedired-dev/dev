		...
		<div class="_gaps_m" style="text-align: center;">
			...
			<MkPagination ref="pagingComponent" :pagination="pagination">
				<template #default="{ items }">
					<div class="_gaps_s">
						<MkInviteCode v-for="item in (items as Fedired.entities.InviteCode[])" :key="item.id" :invite="item" :onDeleted="deleted"/>
					</div>
				</template>
			</MkPagination>
		</div>
...
async function create() {
	const ticket = await fediredApi('invite/create');
	os.alert({
		type: 'success',
...
async function update() {
	currentInviteLimit.value = (await fediredApi('invite/limit')).remaining;
}
