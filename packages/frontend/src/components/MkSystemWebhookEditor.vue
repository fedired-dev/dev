...
async function onSubmitClicked() {
	await loadingScope(async () => {
		...
		try {
			switch (mode.value) {
				case 'create': {
					const result = await fediredApi('admin/system-webhook/create', params);
				...
				case 'edit': {
					...
					const result = await fediredApi('admin/system-webhook/update', { id: id.value!, ...params });
					dialogEl.value?.close();
					emit('submitted', result);
					break;
				}
...
async function test(type: Fedired.entities.SystemWebhook['on'][number]): Promise<void> {
	if (!id.value) {
	...
	await loadingScope(async () => {
		switch (mode.value) {
			case 'edit': {
				...
				try {
					const res = await fediredApi('admin/system-webhook/show', { id: id.value });
