...
<div class="omfetrab" :class="['s' + size, 'w' + width, 'h' + height, { asDrawer, asWindow }]" :style="{ maxHeight: maxHeight ? maxHeight + 'px' : undefined }">
	<input
		...
		@keydown="onKeydown"
	>
	<!-- FirefoxのTabフォーカスが想定外の挙動となるためtabindex="-1"を追加 https://github.com/fedired-dev/fedired/issues/10744 -->
...
const props = withDefaults(defineProps<{
  ...
  pinnedEmojis?: string[];
	...
	targetNote?: Fedired.entities.Note;
}>(), {
	showPinned: true,
});
...
const q = ref<string>('');
const searchResultCustom = ref<Fedired.entities.EmojiSimple[]>([]);
...
watch(q, () => {
	...
	const searchCustom = () => {
		...
		const matches = new Set<Fedired.entities.EmojiSimple>();
...
function canReact(emoji: Fedired.entities.EmojiSimple | UnicodeEmojiDef | string): boolean {
	return !props.targetNote || checkReactionPermissions($i!, props.targetNote, emoji);
}

function filterCategory(emoji: Fedired.entities.EmojiSimple, category: string): boolean {
...
function getKey(emoji: string | Fedired.entities.EmojiSimple | UnicodeEmojiDef): string {
	return typeof emoji === 'string' ? emoji : 'char' in emoji ? emoji.char : `:${emoji.name}:`;
}

function getDef(emoji: string): string | Fedired.entities.EmojiSimple | UnicodeEmojiDef {
	if (emoji.includes(':')) {
...
function chosen(emoji: string | Fedired.entities.EmojiSimple | UnicodeEmojiDef, ev?: MouseEvent) {
