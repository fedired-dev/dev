...
const props = defineProps<{
	modelValue: Fedired.entities.PageBlock & { type: 'note' };
}>();

const emit = defineEmits<{
	(ev: 'update:modelValue', value: Fedired.entities.PageBlock & { type: 'note' }): void;
}>();
...
watch(id, async () => {
	...
	note.value = await fediredApi('notes/show', { noteId: id.value });
}, {
	immediate: true,
});
</script>
