...
const props = withDefaults(defineProps<{
	notification: Fedired.entities.Notification;
	withTime?: boolean;
	full?: boolean;
}>(), {
	withTime: false,
...
type ExportCompletedNotification = Fedired.entities.Notification & { type: 'exportCompleted' };
...
const acceptFollowRequest = () => {
	...
	fediredApi('following/requests/accept', { userId: props.notification.user.id });
};

const rejectFollowRequest = () => {
	if (!('user' in props.notification)) return;
	followRequestDone.value = true;
	fediredApi('following/requests/reject', { userId: props.notification.user.id });
};

function getActualReactedUsersCount(notification: Fedired.entities.Notification) {
