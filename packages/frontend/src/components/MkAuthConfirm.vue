...
const props = defineProps<{
	name?: string;
	icon?: string;
	permissions?: (typeof Fedired.permissions[number])[];
...
const users = ref(new Map<string, Fedired.entities.UserDetailed & { token: string; }>());
...
async function init() {
	...
	if (accountIdsToFetch.length > 0) {
		const usersRes = await fediredApi('users/show', {
