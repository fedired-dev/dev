...
const invitations = ref<Fedired.entities.UserLite[]>([]);
const matchingUser = ref<Fedired.entities.UserLite | null>(null);
const matchingAny = ref<boolean>(false);
const noIrregularRules = ref<boolean>(false);

function startGame(game: Fedired.entities.ReversiGameDetailed) {
...
async function matchHeatbeat() {
	if (matchingUser.value) {
		const res = await fediredApi('reversi/match', {
...
function cancelMatching() {
	if (matchingUser.value) {
		fediredApi('reversi/cancel-match', { userId: matchingUser.value.id });
		matchingUser.value = null;
	} else if (matchingAny.value) {
		fediredApi('reversi/cancel-match', { userId: null });
		matchingAny.value = false;
	}
}

async function accept(user) {
	const game = await fediredApi('reversi/match', {
		userId: user.id,
	});
	if (game) {
		startGame(game);
...
onMounted(() => {
	fediredApi('reversi/invitations').then(_invitations => {
