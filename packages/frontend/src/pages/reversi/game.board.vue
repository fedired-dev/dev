...
const props = defineProps<{
	...
	connection?: Fedired.ChannelConnection<Fedired.Channels['reversiGame']> | null;
}>();
...
const game = ref<Fedired.entities.ReversiGameDetailed & { logs: Reversi.Serializer.SerializedLog[] }>(deepClone(props.game));
		...
		switch (log.operation) {
			case 'put': {
				...
				if (log.player !== engine.value.turn) { // = desyncが発生している
					const _game = await fediredApi('reversi/show-game', {
