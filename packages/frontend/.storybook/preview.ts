...
function loadTheme(applyTheme: typeof import('../src/scripts/theme')['applyTheme']) {
	unobserve();
	const theme = themes[document.documentElement.dataset.fediredTheme];
	if (theme) {
		applyTheme(themes[document.documentElement.dataset.fediredTheme]);
	} else {
	...
	const observer = new MutationObserver((entries) => {
		for (const entry of entries) {
			if (entry.attributeName === 'data-fedired-theme') {
				...
				if (theme) {
					applyTheme(themes[target.dataset.fediredTheme]);
				} else {
