	...
	switch (options.type) {
		...
		case 'lookup': {
			...
			if (targetHost) {
				...
				window.open(`https://joinfedired.org/mi-web/?path=${encodeURIComponent(_path)}`, '_blank', 'noopener');
			}
			break;
		}
		case 'share': {
			...
			if (targetHost) {
				...
				window.open(`https://joinfedired.org/share/?${params}`, '_blank', 'noopener');
			}
			break;
		}
	}
...
async function specifyHostAndOpenRemote(options: OpenOnRemoteOptions): Promise<void> {
	const { canceled, result: hostTemp } = await os.inputText({
		...
		placeholder: 'fedired.example.com',
	});
