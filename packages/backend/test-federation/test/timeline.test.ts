...
describe('Timeline', () => {
	...
	let bobInA: Fedired.entities.UserDetailedNotMe, aliceInB: Fedired.entities.UserDetailedNotMe;
	...
	type TimelineChannel = keyof Fedired.Channels & (`${string}Timeline` | 'antenna' | 'userList' | 'hashtag');
	type TimelineEndpoint = keyof Fedired.Endpoints & (`${string}timeline` | 'antennas/notes' | 'roles/notes' | 'notes/search-by-tag');
	...
	async function postAndCheckReception<C extends TimelineChannel>(
		timelineChannel: C,
		expect: boolean,
		noteParams: Fedired.entities.NotesCreateRequest = {},
		channelParams: Fedired.Channels[C]['params'] = {},
	) {
		let note: Fedired.entities.Note | undefined;
		...
		const params: Fedired.Endpoints[typeof endpoint]['req'] =
			endpoint === 'antennas/notes' ? { antennaId: (channelParams as Fedired.Channels['antenna']['params']).antennaId } :
			endpoint === 'notes/user-list-timeline' ? { listId: (channelParams as Fedired.Channels['userList']['params']).listId } :
			endpoint === 'notes/search-by-tag' ? { query: (channelParams as Fedired.Channels['hashtag']['params']).q } :
			endpoint === 'roles/notes' ? { roleId: (channelParams as Fedired.Channels['roleTimeline']['params']).roleId } :
			{};
	...
	describe('homeTimeline', () => {
		...
		describe('Check reception of remote followee\'s Note', () => {
			 ...
			 * @see https://github.com/fedired-dev/fedired/issues/14083
			 */
			 ...
			 * @see https://github.com/fedired-dev/fedired/issues/14084
			 */
	...
	describe('userList', () => {
		...
		let list: Fedired.entities.UserList;
	...
	describe('roleTimeline', () => {
		...
		let role: Fedired.entities.Role;
	...
	describe.skip('antenna', () => {
		...
		let bobAntenna: Fedired.entities.Antenna;
