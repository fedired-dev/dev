...
function setIframeIdHandler(event: MessageEvent) {
	if (event.data?.type === 'fedired:embedParent:registerIframeId' && event.data.payload?.iframeId != null) {
...
// https://github.com/fedired-dev/fedired/pull/8575#issuecomment-1114239210
// なぜか2回実行されることがあるため、mountするdivを1つに制限する
const rootEl = ((): HTMLElement => {
	const FEDIREDMOUNT_DIV_ID = 'fediredapp';

	const currentRoot = document.getElementById(FEDIREDMOUNT_DIV_ID);
	...
	root.id = FEDIREDMOUNT_DIV_ID;
	document.body.appendChild(root);
	return root;
})();

postMessageToParentWindow('fedired:embed:ready');
...
console.log(i18n.tsx._selfXssPrevention.description3({ link: 'https://joinfedired.org/docs/for-users/resources/self-xss/' }));
//#endregion
