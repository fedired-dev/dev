...
import { getSoundDuration, playFediredSfxFile, soundsTypes, SoundType } from '@/scripts/sound.js';
...
export async function soundSettingsButton(soundSetting: Ref<SoundStore>): Promise<void> {
	...
	const { canceled, result } = await os.form(i18n.ts.sound, {
		...
		soundFile: {
			...
			validate: async (file: Fedired.entities.DriveFile) => {
