...
describe('Webリソース', () => {
	...
	let aliceUploadedFile: fedired.entities.DriveFile | null;
	let alicesPost: fedired.entities.Note;
	let alicePage: fedired.entities.Page;
	let alicePlay: fedired.entities.Flash;
	let aliceClip: fedired.entities.Clip;
	let aliceGalleryPost: fedired.entities.GalleryPost;
	let aliceChannel: fedired.entities.Channel;
	...
	describe('/@:username', () => {
		...
		describe.each([
			...
			test('はHTMLとしてGETできる。', async () => {
				...
				assert.strictEqual(metaTag(res, 'fedired:user-username'), alice.username);
				assert.strictEqual(metaTag(res, 'fedired:user-id'), alice.id);
	...
	describe('/@:user/pages/:page', () => {
		...
		test('はHTMLとしてGETできる。', async () => {
			...
			assert.strictEqual(metaTag(res, 'fedired:user-username'), alice.username);
			assert.strictEqual(metaTag(res, 'fedired:user-id'), alice.id);
			assert.strictEqual(metaTag(res, 'fedired:page-id'), alicePage.id);
	...
	describe('/notes/:id', () => {
		...
		describe.each([
			...
			test('はHTMLとしてGETできる。', async () => {
				...
				assert.strictEqual(metaTag(res, 'fedired:user-username'), alice.username);
				assert.strictEqual(metaTag(res, 'fedired:user-id'), alice.id);
				assert.strictEqual(metaTag(res, 'fedired:note-id'), alicesPost.id);
	...
	describe('/play/:id', () => {
		...
		test('がGETできる。', async () => {
			...
			assert.strictEqual(metaTag(res, 'fedired:user-username'), alice.username);
			assert.strictEqual(metaTag(res, 'fedired:user-id'), alice.id);
			assert.strictEqual(metaTag(res, 'fedired:flash-id'), alicePlay.id);
	...
	describe('/clips/:clip', () => {
		...
		test('がGETできる。', async () => {
			...
			assert.strictEqual(metaTag(res, 'fedired:user-username'), alice.username);
			assert.strictEqual(metaTag(res, 'fedired:user-id'), alice.id);
			assert.strictEqual(metaTag(res, 'fedired:clip-id'), aliceClip.id);
	...
	describe('/gallery/:post', () => {
		...
		test('がGETできる。', async () => {
			...
			// FIXME: fedired:gallery-post-idみたいなmetaタグの設定がない
			// TODO profile.noCrawleの検証
			// TODO twitter:creatorの検証
		});
