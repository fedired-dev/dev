...
import type * as fedired from 'fedired-js';
...
describe('ユーザー', () => {
	...
	const show = async (id: string, me = root): Promise<fedired.entities.UserDetailed> => {
		return successfulApiCall({ endpoint: 'users/show', parameters: { userId: id }, user: me });
	};

	// UserLiteのキーが過不足なく入っている？
	const userLite = (user: fedired.entities.UserLite): Partial<fedired.entities.UserLite> => {
	...
	const userDetailedNotMe = (user: fedired.entities.SignupResponse): Partial<fedired.entities.UserDetailedNotMe> => {
		return stripUndefined({
			...userLite(user),
			url: user.url,
			uri: user.uri,
	...
	const userDetailedNotMeWithRelations = (user: fedired.entities.SignupResponse): Partial<fedired.entities.UserDetailedNotMe> => {
		return stripUndefined({
	...
	const meDetailed = (user: fedired.entities.SignupResponse, security = false): Partial<fedired.entities.MeDetailed> => {
		return stripUndefined({
			...userDetailedNotMe(user),
	...
	let root: fedired.entities.SignupResponse;
	let alice: fedired.entities.SignupResponse;
	let aliceNote: fedired.entities.Note;

	let bob: fedired.entities.SignupResponse;

	// NOTE: これがないと落ちる（bob の updatedAt が null になってしまうため？）
	let bobNote: fedired.entities.Note; // eslint-disable-line @typescript-eslint/no-unused-vars

	let carol: fedired.entities.SignupResponse;

	let usersReplying: fedired.entities.SignupResponse[];

	let userNoNote: fedired.entities.SignupResponse;
	let userNotExplorable: fedired.entities.SignupResponse;
	let userLocking: fedired.entities.SignupResponse;
	let userAdmin: fedired.entities.SignupResponse;
	let roleAdmin: fedired.entities.Role;
	let userModerator: fedired.entities.SignupResponse;
	let roleModerator: fedired.entities.Role;
	let userRolePublic: fedired.entities.SignupResponse;
	let rolePublic: fedired.entities.Role;
	let userRoleBadge: fedired.entities.SignupResponse;
	let roleBadge: fedired.entities.Role;
	let userSilenced: fedired.entities.SignupResponse;
	let roleSilenced: fedired.entities.Role;
	let userSuspended: fedired.entities.SignupResponse;
	let userDeletedBySelf: fedired.entities.SignupResponse;
	let userDeletedByAdmin: fedired.entities.SignupResponse;
	let userFollowingAlice: fedired.entities.SignupResponse;
	let userFollowedByAlice: fedired.entities.SignupResponse;
	let userBlockingAlice: fedired.entities.SignupResponse;
	let userBlockedByAlice: fedired.entities.SignupResponse;
	let userMutingAlice: fedired.entities.SignupResponse;
	let userMutedByAlice: fedired.entities.SignupResponse;
	let userRnMutingAlice: fedired.entities.SignupResponse;
	let userRnMutedByAlice: fedired.entities.SignupResponse;
	let userFollowRequesting: fedired.entities.SignupResponse;
	let userFollowRequested: fedired.entities.SignupResponse;
	...
	beforeAll(async () => {
		...
		}, Promise.resolve([] as fedired.entities.SignupResponse[]));
	...
	test('が作れる。（作りたての状態で自分のユーザー情報が取れる）', async () => {
		...
		const response = await successfulApiCall({
			...
			user: undefined,
		}) as unknown as fedired.entities.SignupResponse; // BUG MeDetailedに足りないキーがある
	...
	test.each([
		{ label: 'ID昇順', parameters: { limit: 5 }, selector: (u: fedired.entities.UserLite): string => u.id },
		{ label: 'フォロワー昇順', parameters: { sort: '+follower' }, selector: (u: fedired.entities.UserDetailedNotMe): string => String(u.followersCount) },
		{ label: 'フォロワー降順', parameters: { sort: '-follower' }, selector: (u: fedired.entities.UserDetailedNotMe): string => String(u.followersCount) },
		{ label: '登録日時昇順', parameters: { sort: '+createdAt' }, selector: (u: fedired.entities.UserDetailedNotMe): string => u.createdAt },
		{ label: '登録日時降順', parameters: { sort: '-createdAt' }, selector: (u: fedired.entities.UserDetailedNotMe): string => u.createdAt },
		{ label: '投稿日時昇順', parameters: { sort: '+updatedAt' }, selector: (u: fedired.entities.UserDetailedNotMe): string => String(u.updatedAt) },
		{ label: '投稿日時降順', parameters: { sort: '-updatedAt' }, selector: (u: fedired.entities.UserDetailedNotMe): string => String(u.updatedAt) },
	...
	test.each([
		{ label: 'Administratorになっている', user: () => userAdmin, me: () => userAdmin, selector: (user: fedired.entities.MeDetailed) => user.isAdmin },
		// @ts-expect-error UserDetailedNotMe doesn't include isAdmin
		{ label: '自分以外から見たときはAdministratorか判定できない', user: () => userAdmin, selector: (user: fedired.entities.UserDetailedNotMe) => user.isAdmin, expected: () => undefined },
		{ label: 'Moderatorになっている', user: () => userModerator, me: () => userModerator, selector: (user: fedired.entities.MeDetailed) => user.isModerator },
		// @ts-expect-error UserDetailedNotMe doesn't include isModerator
		{ label: '自分以外から見たときはModeratorか判定できない', user: () => userModerator, selector: (user: fedired.entities.UserDetailedNotMe) => user.isModerator, expected: () => undefined },
		{ label: '自分から見た場合に二要素認証関連のプロパティがセットされている', user: () => alice, me: () => alice, selector: (user: fedired.entities.MeDetailed) => user.twoFactorEnabled, expected: () => false },
		{ label: '自分以外から見た場合に二要素認証関連のプロパティがセットされていない', user: () => alice, me: () => bob, selector: (user: fedired.entities.UserDetailedNotMe) => user.twoFactorEnabled, expected: () => undefined },
		{ label: 'モデレーターから見た場合に二要素認証関連のプロパティがセットされている', user: () => alice, me: () => userModerator, selector: (user: fedired.entities.UserDetailedNotMe) => user.twoFactorEnabled, expected: () => false },
		{ label: 'サイレンスになっている', user: () => userSilenced, selector: (user: fedired.entities.UserDetailed) => user.isSilenced },
		// FIXME: 落ちる
		//{ label: 'サスペンドになっている', user: () => userSuspended, selector: (user: fedired.entities.UserDetailed) => user.isSuspended },
		{ label: '削除済みになっている', user: () => userDeletedBySelf, me: () => userDeletedBySelf, selector: (user: fedired.entities.MeDetailed) => user.isDeleted },
		// @ts-expect-error UserDetailedNotMe doesn't include isDeleted
		{ label: '自分以外から見たときは削除済みか判定できない', user: () => userDeletedBySelf, selector: (user: fedired.entities.UserDetailedNotMe) => user.isDeleted, expected: () => undefined },
		{ label: '削除済み(byAdmin)になっている', user: () => userDeletedByAdmin, me: () => userDeletedByAdmin, selector: (user: fedired.entities.MeDetailed) => user.isDeleted },
		// @ts-expect-error UserDetailedNotMe doesn't include isDeleted
		{ label: '自分以外から見たときは削除済み(byAdmin)か判定できない', user: () => userDeletedByAdmin, selector: (user: fedired.entities.UserDetailedNotMe) => user.isDeleted, expected: () => undefined },
		{ label: 'フォロー中になっている', user: () => userFollowedByAlice, selector: (user: fedired.entities.UserDetailed) => user.isFollowing },
		{ label: 'フォローされている', user: () => userFollowingAlice, selector: (user: fedired.entities.UserDetailed) => user.isFollowed },
		{ label: 'ブロック中になっている', user: () => userBlockedByAlice, selector: (user: fedired.entities.UserDetailed) => user.isBlocking },
		{ label: 'ブロックされている', user: () => userBlockingAlice, selector: (user: fedired.entities.UserDetailed) => user.isBlocked },
		{ label: 'ミュート中になっている', user: () => userMutedByAlice, selector: (user: fedired.entities.UserDetailed) => user.isMuted },
		{ label: 'リノートミュート中になっている', user: () => userRnMutedByAlice, selector: (user: fedired.entities.UserDetailed) => user.isRenoteMuted },
		{ label: 'フォローリクエスト中になっている', user: () => userFollowRequested, me: () => userFollowRequesting, selector: (user: fedired.entities.UserDetailed) => user.hasPendingFollowRequestFromYou },
		{ label: 'フォローリクエストされている', user: () => userFollowRequesting, me: () => userFollowRequested, selector: (user: fedired.entities.UserDetailed) => user.hasPendingFollowRequestToYou },
	...
	test.each([
		{ label: 'フォロワー昇順', sort: { sort: '+follower' }, selector: (u: fedired.entities.UserDetailedNotMe): string => String(u.followersCount) },
		{ label: 'フォロワー降順', sort: { sort: '-follower' }, selector: (u: fedired.entities.UserDetailedNotMe): string => String(u.followersCount) },
		{ label: '登録日時昇順', sort: { sort: '+createdAt' }, selector: (u: fedired.entities.UserDetailedNotMe): string => u.createdAt },
		{ label: '登録日時降順', sort: { sort: '-createdAt' }, selector: (u: fedired.entities.UserDetailedNotMe): string => u.createdAt },
		{ label: '投稿日時昇順', sort: { sort: '+updatedAt' }, selector: (u: fedired.entities.UserDetailedNotMe): string => String(u.updatedAt) },
		{ label: '投稿日時降順', sort: { sort: '-updatedAt' }, selector: (u: fedired.entities.UserDetailedNotMe): string => String(u.updatedAt) },
