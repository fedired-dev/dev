...
import * as Reversi from 'fedired-reversi';
...
const props = defineProps<{
	game: Fedired.entities.ReversiGameDetailed;
	connection: Fedired.ChannelConnection<Fedired.Channels['reversiGame']>;
}>();

const shareWhenStart = defineModel<boolean>('shareWhenStart', { default: false });

const game = ref<Fedired.entities.ReversiGameDetailed>(deepClone(props.game));
...
function updateSettings(key: typeof Fedired.reversiUpdateKeys[number]) {
...
function onUpdateSettings<K extends typeof Fedired.reversiUpdateKeys[number]>({ userId, key, value }: { userId: string; key: K; value: Fedired.entities.ReversiGameDetailed[K]; }) {
