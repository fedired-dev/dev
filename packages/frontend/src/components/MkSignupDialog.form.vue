...
const emit = defineEmits<{
	(ev: 'signup', user: Fedired.entities.SignupResponse): void;
...
function onChangeEmail(): void {
	...
	fediredApi('email-address/available', {
...
async function onSubmit(): Promise<void> {
	...
	const signupPayload: Fedired.entities.SignupRequest = {
		username: username.value,
	...
	if (res && res.ok) {
		if (res.status === 204 || instance.emailRequiredForSignup) {
			...
			const resJson = (await res.json()) as Fedired.entities.SignupResponse;
