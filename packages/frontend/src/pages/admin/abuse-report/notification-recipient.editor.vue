...
async function onSubmitClicked() {
	await loadingScope(async () => {
		...
		try {
			switch (mode.value) {
				case 'create': {
					await fediredApi('admin/abuse-report/notification-recipient/create', params);
					break;
				}
				case 'edit': {
					// eslint-disable-next-line @typescript-eslint/no-non-null-assertion
					await fediredApi('admin/abuse-report/notification-recipient/update', { id: id.value!, ...params });
					break;
				}
			}
...
async function fetchSystemWebhooks() {
	await loadingScope(async () => {
		systemWebhooks.value = [
			...
			...await fediredApi('admin/system-webhook/list', { }),
		];
	});
}
...
async function fetchModerators() {
	await loadingScope(async () => {
		...
		for (; ;) {
			const res = await fediredApi('admin/show-users', {
				limit: 100,
...
onMounted(async () => {
	await loadingScope(async () => {
		...
		if (mode.value === 'edit') {
			...
			try {
				const res = await fediredApi('admin/abuse-report/notification-recipient/show', { id: id.value });
