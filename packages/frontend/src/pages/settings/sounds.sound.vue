...
import { playFediredSfxFile, soundsTypes, getSoundDuration } from '@/scripts/sound.js';
...
if (type.value === '_driveFile_' && fileId.value) {
	await fediredApi('drive/files/show', {
		fileId: fileId.value,
	}).then((res) => {
...
function listen() {
	...
	playFediredSfxFile(type.value === '_driveFile_' ? {
		type: '_driveFile_',
