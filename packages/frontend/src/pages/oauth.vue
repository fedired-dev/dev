...
const transactionIdMeta = document.querySelector<HTMLMetaElement>('meta[name="fedired:oauth:transaction-id"]');
if (transactionIdMeta) {
	transactionIdMeta.remove();
}

const name = document.querySelector<HTMLMetaElement>('meta[name="fedired:oauth:client-name"]')?.content;
const permissions = document.querySelector<HTMLMetaElement>('meta[name="fedired:oauth:scope"]')?.content.split(' ').filter((p): p is typeof Fedired.permissions[number] => (Fedired.permissions as readonly string[]).includes(p)) ?? [];
