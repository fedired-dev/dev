...
const page = ref<Fedired.entities.Page | null>(null);
const pageId = ref<string | null>(null);
const currentName = ref<string | null>(null);
const title = ref('');
const summary = ref<string | null>(null);
const name = ref(Date.now().toString());
const eyeCatchingImage = ref<Fedired.entities.DriveFile | null>(null);
const eyeCatchingImageId = ref<string | null>(null);
const font = ref('sans-serif');
const content = ref<Fedired.entities.Page['content']>([]);
...
watch(eyeCatchingImageId, async () => {
	if (eyeCatchingImageId.value == null) {
		...
		eyeCatchingImage.value = await fediredApi('drive/files/show', {
...
function save() {
	...
	if (pageId.value) {
		...
		fediredApi('pages/update', options)
			.then(page => {
		...
		fediredApi('pages/create', options)
			.then(created => {
...
function duplicate() {
	...
	fediredApi('pages/create', getSaveOptions()).then(created => {
		pageId.value = created.id;
		currentName.value = name.value.trim();
		os.alert({
			type: 'success',
...
async function init() {
	if (props.initPageId) {
		page.value = await fediredApi('pages/show', {
