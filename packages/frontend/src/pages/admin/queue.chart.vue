...
const delayed = ref(0);
const waiting = ref(0);
const jobs = ref<Fedired.Endpoints[`admin/queue/${ApQueueDomain}-delayed`]['res']>([]);
...
function onStats(stats: Fedired.entities.QueueStats) {
...
function onStatsLog(statsLog: Fedired.entities.QueueStatsLog) {
	const dataProcess: Fedired.entities.QueueStats[ApQueueDomain]['activeSincePrevTick'][] = [];
	const dataActive: Fedired.entities.QueueStats[ApQueueDomain]['active'][] = [];
	const dataDelayed: Fedired.entities.QueueStats[ApQueueDomain]['delayed'][] = [];
	const dataWaiting: Fedired.entities.QueueStats[ApQueueDomain]['waiting'][] = [];
...
onMounted(() => {
	fediredApi(`admin/queue/${props.domain}-delayed`).then(result => {
		jobs.value = result;
	});
