...
const profileProps = ['name', 'createdAt', 'updatedAt', 'fediredVersion', 'settings', 'host'];
...
fediredApi('i/registry/get-all', { scope })
	.then(res => {
		profiles.value = res || {};
	});
...
function validate(profile: any): void {
	...
	if (!profile.fediredVersion) throw new Error('Missing required prop: fediredVersion');
