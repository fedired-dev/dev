			...
			case 'notification':
				switch (action) {
					case 'follow':
						...
						break;
					case 'showUser':
						if ('user' in data.body) client = await swos.openUser(Fedired.acct.toString(data.body.user), loginId);
						break;
					case 'reply':
						...
						switch (data.body.type) {
							...
							default:
								if ('note' in data.body) {
									...
									client = await swos.openUser(Fedired.acct.toString(data.body.user), loginId);
								}
								break;
						}
				}
			...
			default:
				switch (action) {
					case 'markAllAsRead':
						await globalThis.registration.getNotifications()
							.then(notifications => notifications.forEach(n => n.tag !== 'read_notification' && n.close()));
						await get<Pick<Fedired.entities.SignupResponse, 'id' | 'token'>[]>('accounts').then(accounts => {
