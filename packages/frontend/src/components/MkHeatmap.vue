...
async function renderChart() {
	...
	if (props.src === 'active-users') {
		const raw = await fediredApi('charts/active-users', { limit: chartLimit, span: 'day' });
		values = raw.readWrite;
	} else if (props.src === 'notes') {
		if (props.user) {
			const raw = await fediredApi('charts/user/notes', { userId: props.user.id, limit: chartLimit, span: 'day' });
			values = raw.inc;
		} else {
			const raw = await fediredApi('charts/notes', { limit: chartLimit, span: 'day' });
			values = raw.local.inc;
		}
	} else if (props.src === 'ap-requests-inbox-received') {
		const raw = await fediredApi('charts/ap-request', { limit: chartLimit, span: 'day' });
	...
	chartInstance = new Chart(chartEl.value, {
		...
		data: {
			...
			/* @see <https://github.com/fedired-dev/fedired/pull/10365#discussion_r1155511107>
			}] satisfies ChartData[],
			 */
			}],
		},
