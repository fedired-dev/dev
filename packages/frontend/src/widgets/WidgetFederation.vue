...
const charts = ref<Fedired.entities.ChartsInstanceResponse[]>([]);
const fetching = ref(true);

const fetch = async () => {
	const fetchedInstances = await fediredApi('federation/instances', {
		sort: '+latestRequestReceivedAt',
		limit: 5,
	});
	const fetchedCharts = await Promise.all(fetchedInstances.map(i => fediredApiGet('charts/instance', { host: i.host, limit: 16, span: 'hour' })));
