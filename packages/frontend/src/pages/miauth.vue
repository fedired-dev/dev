...
const _permissions = computed(() => {
	return (props.permission ? props.permission.split(',').filter((p): p is typeof Fedired.permissions[number] => (Fedired.permissions as readonly string[]).includes(p)) : []);
});

const authRoot = useTemplateRef('authRoot');

async function onAccept(token: string) {
	await fediredApi('miauth/gen-token', {
