...
const props = withDefaults(defineProps<{
	user: Fedired.entities.UserDetailed,
	full?: boolean,
	large?: boolean,
}>(), {
	full: false,
...
const emit = defineEmits<{
	(_: 'update:user', value: Fedired.entities.UserDetailed): void
}>();
...
function onFollowChange(user: Fedired.entities.UserDetailed) {
...
async function onClick() {
	...
	try {
		if (isFollowing.value) {
			...
			await fediredApi('following/delete', {
				userId: props.user.id,
			});
		} else {
			...
			if (hasPendingFollowRequestFromYou.value) {
				await fediredApi('following/requests/cancel', {
					userId: props.user.id,
				});
				hasPendingFollowRequestFromYou.value = false;
			} else {
				await fediredApi('following/create', {
					userId: props.user.id,
