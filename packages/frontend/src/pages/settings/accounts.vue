...
const accounts = ref<Fedired.entities.UserDetailed[]>([]);

const init = async () => {
	getAccounts().then(accounts => {
		storedAccounts.value = accounts.filter(x => x.id !== $i!.id);

		return fediredApi('users/show', {
...
function menu(account: Fedired.entities.UserDetailed, ev: MouseEvent) {
	os.popupMenu([{
		text: i18n.ts.switch,
...
async function removeAccount(account: Fedired.entities.UserDetailed) {
