...
onMounted(async () => {
	const [_stats, _onlineUsersCount] = await Promise.all([
		fediredApi('stats', {}),
		fediredApiGet('get-online-users-count').then(res => res.count),
	]);
	stats.value = _stats;
	onlineUsersCount.value = _onlineUsersCount;

	fediredApiGet('charts/users', { limit: 2, span: 'day' }).then(chart => {
		usersComparedToThePrevDay.value = stats.value.originalUsersCount - chart.local.total[1];
	});

	fediredApiGet('charts/notes', { limit: 2, span: 'day' }).then(chart => {
