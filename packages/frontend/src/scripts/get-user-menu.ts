...
export function getUserMenu(user: Fedired.entities.UserDetailed, router: IRouter = mainRouter) {
	...
	async function editMemo(): Promise<void> {
		const userDetailed = await fediredApi('users/show', {
			userId: user.id,
		});
