...
export const apiWithDialog = (<E extends keyof Fedired.Endpoints, P extends Fedired.Endpoints[E]['req'] = Fedired.Endpoints[E]['req']>(
	endpoint: E,
	data: P,
	token?: string | null | undefined,
	customErrors?: Record<string, { title?: string; text: string; }>,
) => {
	const promise = fediredApi(endpoint, data, token);
...
export function promiseDialog<T extends Promise<any>>(
	...
	onFailure?: ((err: Fedired.api.APIError) => void) | null,
	text?: string,
): T {
...
export async function selectUser(opts: { includeSelf?: boolean; localOnly?: boolean; } = {}): Promise<Fedired.entities.UserDetailed> {
	return new Promise(resolve => {
...
export async function selectDriveFile(multiple: boolean): Promise<Fedired.entities.DriveFile[]> {
	return new Promise(resolve => {
...
export async function selectDriveFolder(multiple: boolean): Promise<Fedired.entities.DriveFolder[]> {
	return new Promise(resolve => {
		const { dispose } = popup(defineAsyncComponent(() => import('@/components/MkDriveSelectDialog.vue')), {
			type: 'folder',
			multiple,
...
export async function cropImage(image: Fedired.entities.DriveFile, options: {
	aspectRatio: number;
