...
<MkStickyContainer>
	...
	<MkSpacer :contentMax="800">
		...
		<div v-else-if="state === 'posted'" class="_buttonsCenter">
			...
			<MkButton @click="goToFedired">{{ i18n.ts.goToFedired }}</MkButton>
		</div>
	</MkSpacer>
</MkStickyContainer>
</template>
...
// SPECIFICATION: https://joinfedired.org/docs/for-users/features/share-form/
...
const visibilityQuery = urlParams.get('visibility') as typeof Fedired.noteVisibilities[number];
...
const reply = ref<Fedired.entities.Note | undefined>();
const renote = ref<Fedired.entities.Note | undefined>();
const visibility = ref(Fedired.noteVisibilities.includes(visibilityQuery) ? visibilityQuery : undefined);
const localOnly = ref(localOnlyQuery === '0' ? false : localOnlyQuery === '1' ? true : undefined);
const files = ref([] as Fedired.entities.DriveFile[]);
const visibleUsers = ref([] as Fedired.entities.UserDetailed[]);
...
async function init() {
	...
	if (visibility.value === 'specified') {
		...
		await Promise.all(
			[
				...
				...(visibleAccts ? visibleAccts.split(',').map(Fedired.acct.parse) : []),
			]
			// TypeScriptの指示通りに変換する
				.map(q => 'username' in q ? { username: q.username, host: q.host === null ? undefined : q.host } : q)
				.map(q => fediredApi('users/show', q)
					.then(user => {
	...
	try {
		...
		if (replyId) {
			reply.value = await fediredApi('notes/show', {
				noteId: replyId,
			});
		} else if (replyUri) {
			const obj = await fediredApi('ap/show', {
				uri: replyUri,
			});
			if (obj.type === 'Note') {
		...
		if (renoteId) {
			renote.value = await fediredApi('notes/show', {
				noteId: renoteId,
			});
		} else if (renoteUri) {
			const obj = await fediredApi('ap/show', {
		...
		if (fileIds) {
			await Promise.all(
				fileIds.split(',')
					.map(fileId => fediredApi('drive/files/show', { fileId })
						.then(file => {
...
function goToFedired(): void {
	location.href = '/';
}

function onPosted(): void {
	state.value = 'posted';
	postMessageToParentWindow('fedired:shareForm:shareCompleted');
}
