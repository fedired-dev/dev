...
export type ApiRequest<E extends keyof fedired.Endpoints, P extends fedired.Endpoints[E]['req'] = fedired.Endpoints[E]['req']> = {
	endpoint: E,
	parameters: P,
	user: UserToken | undefined,
};

export const successfulApiCall = async <E extends keyof fedired.Endpoints, P extends fedired.Endpoints[E]['req']>(request: ApiRequest<E, P>, assertion: {
	status?: number,
	...
	return res.body as fedired.api.SwitchCaseResponseType<E, P>;
};

export const failedApiCall = async <E extends keyof fedired.Endpoints, P extends fedired.Endpoints[E]['req']>(request: ApiRequest<E, P>, assertion: {
	status: number,
	code: string,
	id: string
}): Promise<void> => {
...
export const api = async <E extends keyof fedired.Endpoints, P extends fedired.Endpoints[E]['req']>(path: E, params: P, me?: UserToken): Promise<{
	status: number,
	headers: Headers,
	body: fedired.api.SwitchCaseResponseType<E, P>
}> => {
	...
	const body = res.headers.get('content-type') === 'application/json; charset=utf-8'
		? await res.json() as fedired.api.SwitchCaseResponseType<E, P>
		: null;
...
export const signup = async (params?: Partial<fedired.Endpoints['signup']['req']>): Promise<NonNullable<fedired.Endpoints['signup']['res']>> => {
...
export const post = async (user: UserToken, params: fedired.Endpoints['notes/create']['req']): Promise<fedired.entities.Note> => {
	const q = params;
...
export const createAppToken = async (user: UserToken, permissions: (typeof fedired.permissions)[number][]) => {
	const res = await api('miauth/gen-token', {
		session: randomUUID(),
		permission: permissions,
	}, user);

	return (res.body as fedired.entities.MiauthGenTokenResponse).token;
};

// 非公開ノートをAPI越しに見たときのノート NoteEntityService.ts
export const hiddenNote = (note: fedired.entities.Note): fedired.entities.Note => {
	const temp: fedired.entities.Note = {
		...note,
		fileIds: [],
		files: [],
		text: null,
...
export const react = async (user: UserToken, note: fedired.entities.Note, reaction: string): Promise<void> => {
...
export const userList = async (user: UserToken, userList: Partial<fedired.entities.UserList> = {}): Promise<fedired.entities.UserList> => {
...
export const page = async (user: UserToken, page: Partial<fedired.entities.Page> = {}): Promise<fedired.entities.Page> => {
...
export const play = async (user: UserToken, play: Partial<fedired.entities.Flash> = {}): Promise<fedired.entities.Flash> => {
...
export const clip = async (user: UserToken, clip: Partial<fedired.entities.Clip> = {}): Promise<fedired.entities.Clip> => {
...
export const galleryPost = async (user: UserToken, galleryPost: Partial<fedired.entities.GalleryPost> = {}): Promise<fedired.entities.GalleryPost> => {
...
export const channel = async (user: UserToken, channel: Partial<fedired.entities.Channel> = {}): Promise<fedired.entities.Channel> => {
...
export const role = async (user: UserToken, role: Partial<fedired.entities.Role> = {}, policies: any = {}): Promise<fedired.entities.Role> => {
...
export const uploadFile = async (user?: UserToken, { path, name, blob }: UploadOptions = {}): Promise<{
	status: number,
	headers: Headers,
	body: fedired.entities.DriveFile | null
}> => {
	...
	const body = res.status !== 204 ? await res.json() as fedired.Endpoints['drive/files/create']['res'] : null;
	return {
		status: res.status,
		headers: res.headers,
		body,
...
export const uploadUrl = async (user: UserToken, url: string): Promise<fedired.entities.DriveFile> => {
...
export function connectStream<C extends keyof fedired.Channels>(user: UserToken, channel: C, listener: (message: Record<string, any>) => any, params?: fedired.Channels[C]['params']): Promise<WebSocket> {
...
export const waitFire = async <C extends keyof fedired.Channels>(user: UserToken, channel: C, trgr: () => any, cond: (msg: Record<string, any>) => boolean, params?: fedired.Channels[C]['params']) => {
...
export function makeStreamCatcher<T>(
	user: UserToken,
	channel: keyof fedired.Channels,
