...
export class ApNoteService {
	 ...
	 * Noteをフェッチします。
	 *
	 * Fediredに対象のNoteが登録されていればそれを返します。
	 */
	@bindThis
	...
	public async createNote(value: string | IObject, resolver?: Resolver, silent = false): Promise<MiNote | null> {
		...
		if (note.source?.mediaType === 'text/x.fediredmarkdown' && typeof note.source.content === 'string') {
			text = note.source.content;
		} else if (typeof note._fedired_content !== 'undefined') {
			text = note._fedired_content;
		...
		if (note._fedired_quote ?? note.quoteUrl) {
			...
			const uris = unique([note._fedired_quote, note.quoteUrl].filter(x => x != null));
	 ...
	 * Noteを解決します。
	 *
	 * Fediredに対象のNoteが登録されていればそれを返し、そうでなければ
	 * リモートサーバーからフェッチしてFediredに登録しそれを返します。
	 */
	@bindThis
