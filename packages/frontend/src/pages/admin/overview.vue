...
const serverInfo = ref<Fedired.entities.ServerInfoResponse | null>(null);
...
const newUsers = ref<Fedired.entities.UserDetailed[] | null>(null);
const activeInstances = shallowRef<Fedired.entities.FederationInstance | null>(null);
...
onMounted(async () => {
	...
	*/
	...
	fediredApiGet('charts/federation', { limit: 2, span: 'day' }).then(chart => {
	...
	fediredApiGet('federation/stats', { limit: 10 }).then(res => {
	...
	fediredApi('admin/server-info').then(serverInfoResponse => {
		serverInfo.value = serverInfoResponse;
	});

	fediredApi('admin/show-users', {
		limit: 5,
		sort: '+createdAt',
	}).then(res => {
		newUsers.value = res;
	...
	fediredApi('federation/instances', {
