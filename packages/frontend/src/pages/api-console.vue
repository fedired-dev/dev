...
fediredApi('endpoints').then(endpointResponse => {
...
function send() {
	...
	fediredApi(endpoint.value as keyof Endpoints, requestBody, requestBody.i || (withCredential.value ? undefined : null)).then(resp => {
		sending.value = false;
...
function onEndpointChange() {
	fediredApi('endpoint', { endpoint: endpoint.value }, withCredential.value ? undefined : null).then(resp => {
		const endpointBody = {};
