...
type PartialAllowedAntenna = Omit<Fedired.entities.Antenna, 'id' | 'createdAt' | 'updatedAt'> & {
	id?: string;
	createdAt?: string;
	updatedAt?: string;
};
...
const src = ref<Fedired.entities.AntennasCreateRequest['src']>(initialAntenna.src);
...
const userLists = ref<Fedired.entities.UserList[] | null>(null);

watch(() => src.value, async () => {
	if (src.value === 'list' && userLists.value === null) {
		userLists.value = await fediredApi('users/lists/list');
	}
});
...
async function deleteAntenna() {
	...
	await fediredApi('antennas/delete', {
