...
export const cli = new Fedired.api.APIClient({ origin, fetch: (...args): Promise<Response> => fetch(...args) });

export async function api<
	E extends keyof Fedired.Endpoints,
	P extends Fedired.Endpoints[E]['req']
>(endpoint: E, userId?: string, params?: P): Promise<Fedired.api.SwitchCaseResponseType<E, P> | undefined> {
	let account: Pick<Fedired.entities.SignupResponse, 'id' | 'token'> | undefined;
	...
	return (cli.request as <E extends keyof Fedired.Endpoints, P extends Fedired.Endpoints[E]['req']>(
		endpoint: E,
		params: P,
		credential?: string | null,
	) => Promise<Fedired.api.SwitchCaseResponseType<E, P>>)(endpoint, params, account?.token);
}
...
export async function openPost(options: { initialText?: string; reply?: Fedired.entities.Note; renote?: Fedired.entities.Note }, loginId?: string): ReturnType<typeof openClient> {
	// クエリを作成しておく
	const url = '/share';
