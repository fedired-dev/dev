 ...
 */
export function playFediredSfx(operationType: OperationType) {
	const sound = defaultStore.state[`sound_${operationType}`];
	playFediredSfxFile(sound).then((succeed) => {
 ...
 */
export async function playFediredSfxFile(soundStore: SoundStore): Promise<boolean> {
	// 連続して再生しない
	...
	canPlay = false;
	return await playFediredSfxFileInternal(soundStore).finally(() => {
		// ごく短時間に音が重複しないように
		setTimeout(() => {
			canPlay = true;
		}, 25);
...
async function playFediredSfxFileInternal(soundStore: SoundStore): Promise<boolean> {
