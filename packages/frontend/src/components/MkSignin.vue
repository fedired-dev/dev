...
const emit = defineEmits<{
	(ev: 'login', v: Fedired.entities.SigninFlowResponse & { finished: true }): void;
}>();
...
const userInfo = ref<null | Fedired.entities.UserDetailed>(null);
const password = ref('');
...
function onPasskeyLogin(): void {
	if (webAuthnSupported()) {
		...
		waiting.value = true;
		fediredApi('signin-with-passkey', {})
			.then((res) => {
...
async function onUsernameSubmitted(username: string) {
	...
	userInfo.value = await fediredApi('users/show', {
		username,
	}).catch(() => null);
...
async function tryLogin(req: Partial<Fedired.entities.SigninFlowRequest>): Promise<Fedired.entities.SigninFlowResponse> {
	const _req = {
		username: req.username ?? userInfo.value?.username,
		...req,
	};

	function assertIsSigninFlowRequest(x: Partial<Fedired.entities.SigninFlowRequest>): x is Fedired.entities.SigninFlowRequest {
	...
	return await fediredApi('signin-flow', _req).then(async (res) => {
		if (res.finished) {
			emit('login', res);
...
async function onLoginSucceeded(res: Fedired.entities.SigninFlowResponse & { finished: true }) {
	if (props.autoSet) {
		await login(res.i);
	}
}
