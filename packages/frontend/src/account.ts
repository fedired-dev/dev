...
type Account = Fedired.entities.MeDetailed & { token: string };
...
export async function openAccountMenu(opts: {
	...
	active?: Fedired.entities.UserDetailed['id'];
	onChoose?: (account: Fedired.entities.UserDetailed) => void;
}, ev: MouseEvent) {
	if (!$i) return;

	async function switchAccount(account: Fedired.entities.UserDetailed) {
	...
	const accountsPromise = fediredApi('users/show', { userIds: storedAccounts.map(x => x.id) });

	function createItem(account: Fedired.entities.UserDetailed) {
		return {
			type: 'user' as const,
			user: account,
...
export function getAccountWithSigninDialog(): Promise<{ id: string, token: string } | null> {
	return new Promise((resolve) => {
		const { dispose } = popup(defineAsyncComponent(() => import('@/components/MkSigninDialog.vue')), {}, {
			done: async (res: Fedired.entities.SigninFlowResponse & { finished: true }) => {
...
export function getAccountWithSignupDialog(): Promise<{ id: string, token: string } | null> {
	return new Promise((resolve) => {
		const { dispose } = popup(defineAsyncComponent(() => import('@/components/MkSignupDialog.vue')), {}, {
			done: async (res: Fedired.entities.SignupResponse) => {
