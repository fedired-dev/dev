...
const props = withDefaults(defineProps<{
	initialFolder?: Fedired.entities.DriveFolder;
	type?: string;
	multiple?: boolean;
...
const emit = defineEmits<{
	(ev: 'selected', v: Fedired.entities.DriveFile | Fedired.entities.DriveFolder): void;
	(ev: 'change-selection', v: Fedired.entities.DriveFile[] | Fedired.entities.DriveFolder[]): void;
	(ev: 'move-root'): void;
	(ev: 'cd', v: Fedired.entities.DriveFolder | null): void;
	(ev: 'open-folder', v: Fedired.entities.DriveFolder): void;
}>();

const loadMoreFiles = shallowRef<InstanceType<typeof MkButton>>();
const fileInput = shallowRef<HTMLInputElement>();

const folder = ref<Fedired.entities.DriveFolder | null>(null);
const files = ref<Fedired.entities.DriveFile[]>([]);
const folders = ref<Fedired.entities.DriveFolder[]>([]);
const moreFiles = ref(false);
const moreFolders = ref(false);
const hierarchyFolders = ref<Fedired.entities.DriveFolder[]>([]);
const selectedFiles = ref<Fedired.entities.DriveFile[]>([]);
const selectedFolders = ref<Fedired.entities.DriveFolder[]>([]);
...
const sortModeSelect = ref<NonNullable<Fedired.entities.DriveFilesRequest['sort']>>('+createdAt');
...
function onStreamDriveFileCreated(file: Fedired.entities.DriveFile) {
	addFile(file, true);
}

function onStreamDriveFileUpdated(file: Fedired.entities.DriveFile) {
...
function onStreamDriveFolderCreated(createdFolder: Fedired.entities.DriveFolder) {
	addFolder(createdFolder, true);
}

function onStreamDriveFolderUpdated(updatedFolder: Fedired.entities.DriveFolder) {
	const current = folder.value ? folder.value.id : null;
...
function onDrop(ev: DragEvent) {
	...
	if (driveFolder != null && driveFolder !== '') {
		...
		fediredApi('drive/folders/update', {
...
function urlUpload() {
	os.inputText({
		...
		fediredApi('drive/files/upload-from-url', {
			url: url,
...
function createFolder() {
	os.inputText({
		...
		fediredApi('drive/folders/create', {
			name: name,
...
function renameFolder(folderToRename: Fedired.entities.DriveFolder) {
	os.inputText({
...
function deleteFolder(folderToDelete: Fedired.entities.DriveFolder) {
	fediredApi('drive/folders/delete', {
...
function upload(file: File, folderToUpload?: Fedired.entities.DriveFolder | null) {
	uploadFile(file, (folderToUpload && typeof folderToUpload === 'object') ? folderToUpload.id : null, undefined, keepOriginal.value).then(res => {
		addFile(res, true);
	});
}

function chooseFile(file: Fedired.entities.DriveFile) {
...
function chooseFolder(folderToChoose: Fedired.entities.DriveFolder) {
...
function unchoseFolder(folderToUnchose: Fedired.entities.DriveFolder) {
	selectedFolders.value = selectedFolders.value.filter(f => f.id !== folderToUnchose.id);
	emit('change-selection', selectedFolders.value);
}

function move(target?: Fedired.entities.DriveFolder | Fedired.entities.DriveFolder['id' | 'parentId']) {
	if (!target) {
		goRoot();
		return;
...
function addFolder(folderToAdd: Fedired.entities.DriveFolder, unshift = false) {
	const current = folder.value ? folder.value.id : null;
...
function addFile(fileToAdd: Fedired.entities.DriveFile, unshift = false) {
	const current = folder.value ? folder.value.id : null;
...
function removeFolder(folderToRemove: Fedired.entities.DriveFolder | string) {
...
function removeFile(file: Fedired.entities.DriveFile | string) {
	const fileId = typeof file === 'object' ? file.id : file;
	files.value = files.value.filter(f => f.id !== fileId);
}

function appendFile(file: Fedired.entities.DriveFile) {
	addFile(file);
}

function appendFolder(folderToAppend: Fedired.entities.DriveFolder) {
	addFolder(folderToAppend);
}

/*
function prependFile(file: Fedired.entities.DriveFile) {
	addFile(file, true);
}

function prependFolder(folderToPrepend: Fedired.entities.DriveFolder) {
...
async function fetch() {
	...
	const foldersPromise = fediredApi('drive/folders', {
		folderId: folder.value ? folder.value.id : null,
		limit: foldersMax + 1,
	}).then(fetchedFolders => {
	...
	const filesPromise = fediredApi('drive/files', {
		folderId: folder.value ? folder.value.id : null,
		type: props.type,
		limit: filesMax + 1,
...
function getMenu() {
	...
	if (folder.value) {
		menu.push({
			...
			icon: 'ti ti-trash',
			action: () => { deleteFolder(folder.value as Fedired.entities.DriveFolder); },
		});
	}
