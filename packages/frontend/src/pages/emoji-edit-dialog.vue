...
const props = defineProps<{
	emoji?: Fedired.entities.EmojiDetailed,
}>();

const emit = defineEmits<{
	(ev: 'done', v: { deleted?: boolean; updated?: Fedired.entities.AdminEmojiUpdateRequest; created?: Fedired.entities.AdminEmojiUpdateRequest }): void,
	(ev: 'closed'): void
}>();
...
const rolesThatCanBeUsedThisEmojiAsReaction = ref<Fedired.entities.Role[]>([]);
const file = ref<Fedired.entities.DriveFile>();

watch(roleIdsThatCanBeUsedThisEmojiAsReaction, async () => {
	rolesThatCanBeUsedThisEmojiAsReaction.value = (await Promise.all(roleIdsThatCanBeUsedThisEmojiAsReaction.value.map((id) => fediredApi('admin/roles/show', { roleId: id }).catch(() => null)))).filter(x => x != null);
}, { immediate: true });
...
async function removeRole(role: Fedired.entities.RoleLite, ev: Event) {
