...
type PostFormPropsForAsUi = {
	text: string;
	cw?: string;
	visibility?: (typeof Fedired.noteVisibilities)[number];
	localOnly?: boolean;
};
...
function getPostFormProps(form: values.VObj): PostFormPropsForAsUi {
	...
	return {
		text: text.value,
		cw: cw?.value,
		visibility: (visibility?.value && (Fedired.noteVisibilities as readonly string[]).includes(visibility.value)) ? visibility.value as typeof Fedired.noteVisibilities[number] : undefined,
