...
describe('API', () => {
	...
	test('with params', async () => {
		...
		fetchMock.mockResponse(async (req) => {
			const body = await req.json();
			if (req.method == 'POST' && req.url == 'https://fedired.test/api/notes/show') {
	...
	test('multipart/form-data', async () => {
		fetchMock.resetMocks();
		fetchMock.mockResponse(async (req) => {
			if (req.method == 'POST' && req.url == 'https://fedired.test/api/drive/files/create') {
		...
		expect(getFetchCall(fetchMock.mock.calls[0])).toEqual({
			url: 'https://fedired.test/api/drive/files/create',
			method: 'POST',
	...
	test('204 No Content で null が返る', async () => {
		fetchMock.resetMocks();
		fetchMock.mockResponse(async (req) => {
			if (req.method == 'POST' && req.url == 'https://fedired.test/api/reset-password') {
				return { status: 204 };
			} else {
				return { status: 404 };
			}
