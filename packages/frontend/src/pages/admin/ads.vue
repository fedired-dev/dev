...
const ads = ref<Fedired.entities.Ad[]>([]);
...
fediredApi('admin/ad/list', { publishing: publishing }).then(adsResponse => {
	if (adsResponse != null) {
...
function more() {
	fediredApi('admin/ad/list', { untilId: ads.value.reduce((acc, ad) => ad.id != null ? ad : acc).id, publishing: publishing }).then(adsResponse => {
