...
const emit = defineEmits<{
	(ev: 'ok', selected: Fedired.entities.UserDetailed): void;
	(ev: 'cancel'): void;
	(ev: 'closed'): void;
}>();
...
const username = ref('');
const host = ref('');
const users = ref<Fedired.entities.UserLite[]>([]);
const recentUsers = ref<Fedired.entities.UserDetailed[]>([]);
const selected = ref<Fedired.entities.UserLite | null>(null);
...
function search() {
	...
	fediredApi('users/search-by-username-and-host', {
...
async function ok() {
	...
	const user = await fediredApi('users/show', {
