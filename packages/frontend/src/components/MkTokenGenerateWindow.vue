...
const props = withDefaults(defineProps<{
	...
	initialPermissions?: (typeof Fedired.permissions)[number][] | null;
}>(), {
	title: null,
	information: null,
	initialName: null,
...
const defaultPermissions = Fedired.permissions.filter(p => !p.startsWith('read:admin') && !p.startsWith('write:admin'));
const adminPermissions = Fedired.permissions.filter(p => p.startsWith('read:admin') || p.startsWith('write:admin'));

const dialog = shallowRef<InstanceType<typeof MkModalWindow>>();
const name = ref(props.initialName);
const permissionSwitches = ref(<Record<(typeof Fedired.permissions)[number], boolean>>{});
const permissionSwitchesForAdmin = ref(<Record<(typeof Fedired.permissions)[number], boolean>>{});
