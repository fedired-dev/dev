...
const meta = ref<Fedired.entities.AdminMetaResponse | null>(null);
const instance = ref<Fedired.entities.FederationInstance | null>(null);
...
if (iAmModerator) {
	watch(moderationNote, async () => {
		...
		await fediredApi('admin/federation/update-instance', { host: instance.value.host, moderationNote: moderationNote.value });
	});
}

async function fetch(): Promise<void> {
	if (iAmAdmin) {
		meta.value = await fediredApi('admin/meta');
	}
	instance.value = await fediredApi('federation/show-instance', {
		host: props.host,
	});
...
async function toggleBlock(): Promise<void> {
	...
	await fediredApi('admin/update-meta', {
...
async function stopDelivery(): Promise<void> {
	...
	await fediredApi('admin/federation/update-instance', {
...
function refreshMetadata(): void {
	if (!iAmModerator) return;
	if (!instance.value) throw new Error('No instance?');
	fediredApi('admin/federation/refresh-remote-instance-metadata', {
		host: instance.value.host,
	});
	os.alert({
