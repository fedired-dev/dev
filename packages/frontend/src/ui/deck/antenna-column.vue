...
import type { entities as FediredEntities } from 'fedired-js';
...
async function setAntenna() {
	const antennas = await fediredApi('antennas/list');
	const { canceled, result: antenna } = await os.select<FediredEntities.Antenna | '_CREATE_'>({
	...
	if (antenna === '_CREATE_') {
		const { dispose } = os.popup(defineAsyncComponent(() => import('@/components/MkAntennaEditorDialog.vue')), {}, {
			created: (newAntenna: FediredEntities.Antenna) => {
...
function onNote() {
	sound.playFediredSfxFile(soundSetting.value);
}
