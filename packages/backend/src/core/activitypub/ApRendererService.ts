...
export class ApRendererService {
	...
	public async renderNote(note: MiNote, dive = true): Promise<IPost> {
		...
		const { content, noFediredContent } = this.apMfmService.getNoteHtml(note, apAppend);
		...
		return {
			...
			...(noFediredContent ? {} : {
				_fedired_content: text,
				source: {
					content: text,
					mediaType: 'text/x.fediredmarkdown',
				},
			}),
			_fedired_quote: quote,
			quoteUrl: quote,
			published: this.idService.parse(note.id).date.toISOString(),
			to,
			cc,
	...
	public async renderPerson(user: MiLocalUser) {
		...
		const person: any = {
			...
			_fedired_summary: profile.description,
			_fedired_followedMessage: profile.followedMessage,
			_fedired_requireSigninToViewContents: user.requireSigninToViewContents,
			_fedired_makeNotesFollowersOnlyBefore: user.makeNotesFollowersOnlyBefore,
			_fedired_makeNotesHiddenBefore: user.makeNotesHiddenBefore,
